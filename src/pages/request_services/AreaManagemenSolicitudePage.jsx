...
const AreaManagemenSolicitudePage = (props) => {
  ...
  useMemo(() => {
    columns_table = [
      ...
      {
        Header: 'Teléfono',
        accessor: props1 => [props1.housing.phone_client_administration]
      },
      {
        Header: 'Email',
        accessor: props1 => [props1.housing.email_client_administration]
      },
      {
        Header: 'Prioridad',
  ...
  const handleSubmit = e => {
    ...
    axios.post(API_URL+'area_ss_by_filter',objectPost).then(result => {
  ...
  const modifyRegister = dataUpdate => {
    props.history.replace('/request/area/managment/form/'+dataUpdate.id)
  }

  const exportToExcel = () => {

    let wb = xlsx.utils.book_new()
    let bodyTable = [['Nr.SS','Fecha Emisión','Proyecto','Cliente','Prioridad','Nro.Papeleta','Estado','Motivo Cierre','Fecha Inspección','Días Diferencia','Fecha Ejecución','Fecha(N/C/A)','Días Diferencia','Total Días','Total Presupuesto']]

    wb.Props = {
      Title: "Reporte de Solicitudes de Area",
    ...
    saveAs(new Blob([datos1],{type:"application/octet-stream"}), 'Reporte de Seguimientos de Area'+moment().format('DD-MM-YYYY')+'.xlsx')
    ...
    <Container fluid={true}>
      <Row className="containerDiv">
        <Col sm={12} md={12} lg={12}>
          <Row className="">
            <Col sm={12} md={12} lg={12}>
              <h4 className="text-center title_principal">   GESTIÓN SOLICITUDES DE SERVICIO - ÁREAS COMUNES</h4>
              <hr/>
            </Col>
          </Row>
...
AreaManagemenSolicitudePage.defaultProps = {
  inputSS:{
    type: 'text',
    required: false,
    name: 'number_ss',
...
export default AreaManagemenSolicitudePage
