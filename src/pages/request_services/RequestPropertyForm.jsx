...
const RequestPropertyForm = (props) => {
  ...
  const getOwnerships = id => {
    axios.get(API_URL+'masters_ownership_by_housing_complexe/'+id).then(result => {
    ...
    <Container fluid={true} className="containerDiv">
      <Row>
        <Col sm={12} md={12} lg={12}>
          <h4 className="text-center title_principal">Solicitud de Servicio PostVenta</h4>
        </Col>
      </Row>
      <br/>
      <Container>
...
RequestPropertyForm.defaultProps = {
  inputDateRequest:{
    type: 'date',
    required: true,
    name: 'date_request',
