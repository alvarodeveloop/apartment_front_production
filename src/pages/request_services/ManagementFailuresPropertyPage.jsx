...
const ManagementFailuresPropertyPage = (props) => {
  ...
  useMemo(() => {
    columns_table = [
      ...
      {
        Header: 'Depto.',
        accessor: props1 => [props1.ownership_ss.ownership.number]
      },
      {
        Header: 'Módelo',
        accessor: props1 => [props1.ownership_ss.ownership.models.name]
      },
      {
        Header: 'Proyecto',
      ...
      {
        Header: 'Prioridad',
        accessor: props1 => [props1.ownership_ss.priority.name]
      },
      {
        Header: 'Nro.Papeleta',
        accessor: props1 => [props1.ownership_ss.nro_ballot]
      },
      {
        Header: 'Seguimiento',
        Cell: props1 => {
  ...
  const fetchData = (type = false) => {
    ...
    let promises1 = [
      axios.get(API_URL+'ownership_failure_ss'),
    ]
  ...
  const handleSubmit = e => {
    ...
    axios.post(API_URL+'ownership_failure_ss_by_filter',objectPost).then(result => {
  ...
  const exportToExcel = () => {
    ...
    let bodyTable = [['Nr.SS','Fecha Emisión','Cliente','Departamento','Modelo','Proyecto','Recinto','Tipologia de Falla','Falla Puntual','Motivo Falla','Prioridad','Nro.Papeleta','Estado','Motivo Cierre','Fecha Cierre','Valor Materiales','Valor Mano de Obra','Valor Tercero']]

    wb.Props = {
      Title: "Reporte de Fallas de Propiedad",
    ...
    failures.forEach((v,i) => {
      ...
      bodyTable.push([
        v.ownership_ss.id,
        moment(v.ownership_ss.date_request).format('DD-MM-YYYY'),
        v.ownership_ss.client ? v.ownership_ss.client.name+' '+v.ownership_ss.client.last_names : '',
        v.ownership_ss.ownership.number,
        v.ownership_ss.ownership.models.name,
        v.ownership_ss.housing.name,
        v.precint.name,
        v.tipology.name,
        v.point.name,
        v.reason_failure ? v.reason_failure.name : '',
        v.ownership_ss.priority.name,
        v.ownership_ss.nro_ballot,
        v.status.status,
        v.reason_close ? v.reason_close.name : '',
        v.date_close ? moment(v.date_close).format('DD-MM-YYYY') : '',
        v.material_cost ? showPriceWithDecimals(v.material_cost) : '',
        v.workforce_cost ? showPriceWithDecimals(v.workforce_cost) : '',
        v.third_party_cost ? showPriceWithDecimals(v.third_party_cost) : '',
      ])
    })
    ...
    saveAs(new Blob([datos1],{type:"application/octet-stream"}), 'Reporte de Fallas de Propiedad'+moment().format('DD-MM-YYYY')+'.xlsx')
  }
    ...
    <Container fluid={true}>
      <Row  className="containerDiv">
        <Col sm={12} md={12} lg={12}>
          <Row className="">
            <Col sm={12} md={12} lg={12}>
              <h4 className="text-center title_principal">SEGUIMIENTO DE FALLAS POR SOLICITUDES DE SERVICIO</h4>
              <hr/>
            </Col>
          </Row>
          <Row>
