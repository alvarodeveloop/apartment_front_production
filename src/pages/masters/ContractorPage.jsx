...
const ContractorPage = (props) => {
  ...
  const confirmDeleteRegister = id => {
    axios.delete(API_URL+'master_contractor/'+id).then(result => {
  ...
  const fetchData = () => {
    axios.get(API_URL+'master_contractor').then(result => {
      setData(result.data)
    }).catch(err => {
      if(err.response){
        toast.error(err.response.data.message)
  ...
  const goToForm = () => {
    props.history.replace('/masters/contractor/form')
  }

  const modifyRegister = datos => {
    props.history.replace('/masters/contractor/form/'+datos.id)
  }
    ...
    <Container fluid={true}>
      <Row className="containerDiv">
        <Col sm={12} md={12} lg={12} xs={12} className="">
          <h4 className="title_principal">Contratistas</h4>
          <hr/>
        </Col>
        <Col sm={12} md={12} lg={12}>
          <Row className="justify-content-center">
            <Col sm={4} md={4} lg={4}>
              <Button variant="secondary" size="sm" block={true} onClick={goToForm}>Crear Contratista <FaPlusCircle/></Button>
            </Col>
          </Row>
        </Col>
        <Col sm={12} md={12} lg={12}>
          <br/>
          <Table data={data} columns={contractor_columns} />
        </Col>
      </Row>
    </Container>
  )
