...
const BrandPage = (props) => {
  ...
  const confirmDeleteRegister = id => {
    axios.delete(API_URL+'master_brand/'+id).then(result => {
  ...
  const fetchData = () => {
    axios.get(API_URL+'master_brand').then(result => {
  ...
  const onSubmit = e => {
    ...
    if(objectPost.id){
      axios.put(API_URL+'master_brand/'+objectPost.id,objectPost).then(result => {
      ...
      axios.post(API_URL+'master_brand',objectPost).then(result => {
    ...
    <Container>
      <Row className="containerDiv">
        <Col sm={12} md={12} lg={12} xs={12} className="">
          <h4 className="title_principal">Marcas</h4>
          <hr/>
        </Col>
        ...
        <Col sm={12} md={12} lg={12} xs={12}>
          <br/><br/>
          <Table data={brands} columns={brand_columns} />
        </Col>
      </Row>
    </Container>
  )
