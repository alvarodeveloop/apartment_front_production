...
const ServiceOriginPage = (props) => {
  ...
  const confirmDeleteRegister = id => {
    axios.delete(API_URL+'params_service_origin/'+id).then(result => {
  ...
  const fetchData = () => {
    axios.get(API_URL+'params_service_origin').then(result => {
      setData(result.data)
    }).catch(err => {
      if(err.response){
        toast.error(err.response.data.message)
  ...
  const onSubmit = e => {
    ...
    if(objectPost.id){
      axios.put(API_URL+'params_service_origin/'+objectPost.id,objectPost).then(result => {
      ...
      axios.post(API_URL+'params_service_origin',objectPost).then(result => {
    ...
    <Container>
      <Row className="containerDiv">
        <Col sm={12} md={12} lg={12} xs={12} className="">
          <h4 className="title_principal">Origenes de Servicio</h4>
          <hr/>
        </Col>
...
ServiceOriginPage.defaultProps = {
  inputName: {
    type: 'text',
    required: true,
    name: 'name',
...
export default ServiceOriginPage
