...
const SolutionsPage = (props) => {
  ...
  const confirmDeleteRegister = id => {
    axios.delete(API_URL+'params_manage_problems_solutions/'+id).then(result => {
  ...
  const onSubmit = e => {
    ...
    if(objectPost.id){
      axios.put(API_URL+'params_manage_problems_solutions/'+objectPost.id,objectPost).then(result => {
      ...
      axios.post(API_URL+'params_manage_problems_solutions',objectPost).then(result => {
    ...
    <Container>
      <Row className="containerDiv">
        <Col sm={12} md={12} lg={12} xs={12} className="">
          <h4 className="title_principal">Soluciones de Fallas</h4>
          <hr/>
        </Col>
